...
{
    private readonly Customer _customer = new Customer("Bruno Gomes", "bruno@gmail.com");
    ...
    {
        var _discountExp = new Discount(10, DateTime.Now.AddDays(-10));

        Order order = new Order(_customer, 10, _discountExp);
        order.AddItem(_product, 5);
    ...
    {
        Order order = new Order(_customer, 10, _discount);//Valor 0
        order.AddItem(_product, 6); //Valor 70
        Assert.AreEqual(order.Total(), 60);
    }
    ...
    {
        Order order = new Order(null, 10, _discount);//Valor 0
